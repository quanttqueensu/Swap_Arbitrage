...
from signal_data import build_signal_data, clean_maturity
...
def load_signal_frame(refresh_signals: bool = False) -> pd.DataFrame:
    if refresh_signals:
        if not RAW_PRICE_DATA_FILE.exists():
            raise FileNotFoundError(f"Missing {RAW_PRICE_DATA_FILE}. Run `python raw_price_data.py` first.")

        return clean_backtest_frame(build_signal_data(save=False))

    if not SIGNAL_DATA_FILE.exists():
        raise FileNotFoundError(f"Missing {SIGNAL_DATA_FILE}. Run `python signal_data.py` first.")

    return clean_backtest_frame(pd.read_csv(SIGNAL_DATA_FILE))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--refresh-signals", action="store_true", help="Recompute signal/risk data in memory before the run.")
