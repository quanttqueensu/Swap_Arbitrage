...
from data_pipeline.historical_data.canonicalize import (
...
class CanonicalizerTests(unittest.TestCase):
    ...
    def test_rates_emit_only_present_consumed_series_without_zero_or_proxy(self) -> None:
        path = self.fixture(
            ...
            RATE_HEADER,
            [["2026-08-01", *("4" for _ in range(6)), "", "4", "4.25", "4", "4", "4", "4", "", "4.34"]],
        )
        partitions = canonicalize_rates(path)
    ...
    def test_futures_build_expiry_aware_eris_ids_blank_settlement_dv01_and_proxy_risk(self) -> None:
        ...
        self.assertEqual(result.settlements_by_year[2026], [
        ...
        self.assertEqual(result.risk_by_year[2026], [
        ...
        self.assertEqual(validate_csv(SCHEMAS["historical_futures_settlements"], self.write_partition("settlements.csv", result.settlements_by_year[2026], "historical_futures_settlements")), 2)
        self.assertEqual(validate_csv(SCHEMAS["contract_risk"], self.write_partition("risk.csv", result.risk_by_year[2026], "contract_risk")), 2)
    ...
    def test_canonicalizers_fail_closed_for_bad_headers_duplicates_missing_and_invalid_numbers(self) -> None:
        cases = [
            ("unknown", [*RATE_HEADER, "extra"], [["2026-08-01", *("4" for _ in range(15)), "x"]]),
            ("duplicate", RATE_HEADER, [["2026-08-01", *("4" for _ in range(15))], ["2026-08-01", *("4" for _ in range(15))]]),
        ...
        for name, header, rows in cases:
            with self.subTest(name=name), self.assertRaises(CanonicalizationError):
    ...
    def test_rejects_noncanonical_dates_and_undocumented_eris_shorthand(self) -> None:
        for bad_date in ("20260801", "2026-W31-6"):
            with self.subTest(bad_date=bad_date):
                with self.assertRaises(CanonicalizationError):
                    canonicalize_rates(self.fixture(f"{bad_date}.csv", RATE_HEADER, [[bad_date, *("4" for _ in range(15))]]))
