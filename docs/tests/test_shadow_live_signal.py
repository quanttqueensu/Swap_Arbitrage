...
class ShadowLiveSignalTests(unittest.TestCase):
    def test_complete_cycle_writes_two_rows_and_never_exposes_executable_target(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.executable_target_changed)
            ...
            self.assertNotEqual(
                result.hypothetical_target.maturities["2Y"].swap_quantity, 0
            )
            self.assertLessEqual(
                result.hypothetical_target.gross_target_dv01, Decimal("10000")
            )
    ...
    def test_duplicate_poll_appends_observations_and_restart_restores_state(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            first_runner = ShadowLiveSignalRunner(**kwargs)
            first = first_runner.run_once(NOW)
            second = first_runner.run_once(NOW)
            restarted = ShadowLiveSignalRunner(**kwargs).run_once(NOW)
    ...
    def test_missing_5y_quote_blocks_only_5y_hypothetical_exposure(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.hypothetical_target.maturities["5Y"].swap_quantity, 0)
            self.assertNotEqual(result.hypothetical_target.maturities["2Y"].swap_quantity, 0)
            self.assertFalse(result.executable_target_changed)
