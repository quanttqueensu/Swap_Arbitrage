...
Automated integration tests never connect to external data providers or IBKR.
...
Inspect the source-coverage matrix. Approve only when every consumed field has
an observed/derived/assumed/unavailable label and proxy results cannot be
mistaken for the full hypothesis.
...
- recovery path and staging destination.

Approval authorizes staging only. It does not authorize deletion of sources or
legacy results.

### MG4 — Source adapters and staged canonical data

Inspect:

- source-neutral local, IBKR, FRED, and CME adapter samples against their
  approved source documentation, including the provider identity retained in
  each row's `source` field;
- IBKR paper-recording schema generated by fakes;
- credential/account redaction;
- staged canonical row counts, ranges, hashes, duplicates, source-neutral
  paths, schema headers/keys, provenance, and spot checks;
- deterministic rerun comparison;
- migration report and recovery process.
...
- Data migration failures leave original files untouched and use the approved
  staging recovery process.
- Historical run outputs, configs, and manifests are immutable; corrections
