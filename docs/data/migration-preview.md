...
| `treasury_rates` | 1 `data/treasury_rates.csv` | regenerate | `rates_YYYY.csv` and `daily_market_YYYY.csv` | 2,143 dates expand from 16 wide columns to one long row per present consumed series | Validate units/publication clocks and reconcile spot values; retain current file |
